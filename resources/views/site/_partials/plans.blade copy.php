                ...
                <div x-show="tab === '{{ $plan->billing_cycle }}'" x-transition
                    ...
                    <div>
                        ...
                        @elseif (!$user->subscribed('default'))
                            <a href="{{ route('choice.plan', $plan->slug) }}"
                                class="block px-4 py-3 font-semibold text-center text-white bg-blue-600 rounded hover:bg-blue-700">
                                Escolher Plano
                            </a>
                        @elseif ($currentPlanId === $plan->stripe_id)
                            <span class="block w-full px-4 py-3 text-center btn-success">
                                Plano Atual
                            </span>
                        @else
                            <a href="{{ route('choice.plan', $plan->slug) }}"
                                class="block px-4 py-3 font-semibold text-center text-white bg-green-600 rounded hover:bg-green-700">
                                Alterar Plano
                            </a>
                        @endif
                    </div>
